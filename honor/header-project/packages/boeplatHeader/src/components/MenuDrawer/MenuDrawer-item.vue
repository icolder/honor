  ...
  <a-row :gutter="10" :type="'flex'" class="clx-ant-col-5-box">
    <a-col :span="6" v-for="item in twoList" :key="item.id" style="display: flex" class="menu-drawer-item">
      <div class="menu-drawer-item-details" @click="openTarget(item)">
        ...
        <img class="menu-drawer-item-details-ykt" v-if="item.isOpen - 0 === 1" :src="openLogo" />
      </div>
    </a-col>
  </a-row>
</template>
...
<script>
import openLogo from "../../assets/header/is-open.svg";
...
export default {
  ...
  methods: {
    openTarget(item) {
      ...
      if (item.isOpen === 1) {
        // 已开通
        url = `${this.boeEnv.VUE_APP_LOGIN_URL}client_id=${clientId}&response_type=${responseType}&redirect_uri=${redirectUri}`;
      } else {
        if (item.url) {
          url = this.boeEnv.VUE_APP_ZCLOUD_URL + '?productId='+item.productId +'#/'+item.url;
        } else {
          this.$message.warning('url数据异常！');
          return
        }
      }
      window.open(url);
